  ...
  return {
    ...
    transferFunds: function (request, callback) {
      ...
      if (request.lockReceipt) {
        ...
        // Is there enough money in the lock?
        if (locks[request.lockId] < request.value) {
          callback(new Error('Insufficient locked funds'));
          return;
        }
        ...
        // First, check if there are sufficient funds
        var balance = config.balances[request.from];
        if (balance < request.value) {
          callback(new Error('Insufficient funds'));
          return;
        }
